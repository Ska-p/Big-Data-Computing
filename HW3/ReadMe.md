...
+ The approximate second moment $\tilda{F}2 of $\Sigma_R$ using count sketch, also normalized by dividing it by $|\Sigma_R|^2$.
...
largest frequency of the items of $\Sigma_R$ Recall that if $\tilda{f}_u$ is the estimated frequency for $u$, the relative error of is $\frac{|f_u−\tilda{f}_u|}{f_u}$.
